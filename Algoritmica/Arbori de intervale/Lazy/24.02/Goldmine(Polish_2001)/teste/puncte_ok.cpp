...
#include <iostream>
using namespace std;
// Nr max de puncte din dreptunghi, Echivalent  cu nr max de segmente care se intersecteaza pe OY
...
void Update(int nod, int l, int r)
{
	if ( L <= l && r <= R )
	{
		Cnt[nod] += X; Nmax[nod] += X;
		return;
	}
	if ( L <= mid ) Update(2 * nod, l, mid);
	if ( R > mid ) Update(2*nod + 1, mid + 1, r);
	Nmax[nod] = max(Nmax[2*nod], Nmax[2*nod + 1]) + Cnt[nod];
}
