...
int a[2*DIM + 2], FPoz[DIM + 1], SPoz[DIM + 1]; // Poz[x] - poz primei aparitii a lui x in sirul a
...
int pmin[DIM + 1][17];        // pmin[i][j] pozitia minimului in sirul niv, incepand de la poz i si de lungime 2^j  (RMQ)
...
//	Debug();
    ...
    for ( int i = 1; i <= m; ++i )
    {
        ...
        if ( op == 0 )
        {
            pa = FPoz[x];   // pozitia de update  in sirul a   
            v = y;    // valoarea de update
            val[x] = y;
            Update(1, 1, 1000);
            
            pa = SPoz[x];   // pozitia de update  in sirul a   
            v = -y;    // valoarea de update
            Update(1, 1, 1000);
        }    
        else
        {
			...
			int k = log2(j - i + 1); //(int)floor(log(j - i)/log(2)); // log2(a) = ln(a)/ln(2)
