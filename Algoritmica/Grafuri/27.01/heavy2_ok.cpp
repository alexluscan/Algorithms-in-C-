...
public:
    ST(const VI& values)
    {
        ...
        tree = VI(2 * N, 0);    // cel mult 4 n  (sau -Inf, daca valorile pot fi negative
        for (size_t i = 0; i < values.size(); ++i) // valorile innitiale puse pe poz [n, .. 2n -1]
            tree[N + i] = values[i];
        for (int x = N - 1; x > 0; --x)
    ...
    int Query(int l, int r) const  // primeste pozitii (indexate de la 0 in path)
    {
        l += N;  r += N;
        int vmax = 0;
        while (l <= r) 
...
int N;
VI v, t, nr, h, hmin;
VI pId;  // pId[x] id-ul lantului pe care se gasete nodul x
...
void BuildHeavyPath() 
{
	...
	for (size_t p = 0; p < paths.size(); ++p) 
	{
        ...
        st.emplace_back(pathValues);
    }
}

int Query(int x, int y)  // acest query primeste noduri
{
    if (pId[x] == pId[y]) 
    {
        if (pos[x] > pos[y])
