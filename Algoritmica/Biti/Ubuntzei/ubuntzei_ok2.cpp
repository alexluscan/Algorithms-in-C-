...
VVI D, dp;   // D[i] - // D[i] - vector distantele  minime de la C[i] la restul nodurilor
             // dp[i][j] - dist minima a unui drum care porneste din 1 si se termina cu nodul C[j], 
             //            daca drumul contine o submultime i de noduri din cele speciale, si j este inclus in i
int n, m, K;
vector<vector<PII>> G;
...
int main()
{
	...
	else
	{
		...
		for (int i = 1; i < (1 << K); ++i)    // pt fiecare submultime i de pozitii din sirul nodurilor {C[0], ..., C[K - 1] }
			for (int j = 0; j < K; ++j)       // pt fiecare 
				if (i & (1 << j))             // masca i contine bitul j
					for (int k = 0; k < K; ++k)
						if ( i & (1 << k) )   // masca i contine bitul k
...
void Dijkstra(int S, VI& d)
{
	...
	for (int x, dx, y, w; !Q.empty(); Q.pop())
	{
		x = Q.top().nod; dx = Q.top().dist;
