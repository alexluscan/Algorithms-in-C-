...
//Method: Disjoint sets + Pascal triangle with two lines, only necessary columns
...
int getPascalTriangle(int line, int column)
{
	...
	FOR(i, 1, line)
	{
		...
		c[actualLine][0] = 1;
		c[actualLine][i] = 1;
		FOR(j, 1, min(column, i - 1)) c[actualLine][j] = (c[1 - actualLine][j - 1] + c[1 - actualLine][j]) % BASE;
	}
