...
//Time complexity: O(nrCars * log(nrCars) + maxCarsInACompany ^ 2)
//Space complexity: O(nrCars + maxCarsInACompany ^ 2)
//Method: Disjoint sets + Pascal triangle
...
void buildPascalTriangle(int nrLines)
{
	c.RS(nrLines + 1);
	FOR(i, 1, nrLines)
	{
		...
		c[i][0] = 1;
		c[i][i] = 1;
		FOR(j, 1, i - 1) c[i][j] = (c[i - 1][j - 1] + c[i - 1][j]) % BASE;
	}
}
...
int solve()
{
	...
	{
		if (f <= companySizes[i]) solution = (solution * c[companySizes[i]][f]) % BASE;
		f = (f + r) % k;
	}
	return solution;
}
...
int main()
{
	...
	//Solve
	getCompanies();
	buildPascalTriangle(*max_element(ALL(companySizes)));
