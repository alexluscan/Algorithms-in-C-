...
struct LiChaoTree {
	...
	{
		auto bl = comp(seg(xs[l]), a[nd](xs[l]));     
		auto br = comp(seg(xs[r]), a[nd](xs[r]));
		...
		{
			int mid = (l + r) >> 1;
			bool mj = comp(seg(xs[mid]), a[nd](xs[mid]));
			if (mj) swap(seg, a[nd]);

			if (bl != mj) 
				insert(seg, 2 * nd, l, mid); // daca s-a facut swap, trimitem a[nd] spre fiu, daca nu trimtem seg
			else 
